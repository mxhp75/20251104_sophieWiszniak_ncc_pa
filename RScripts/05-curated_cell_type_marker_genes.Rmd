...
library(patchwork)
library(pheatmap)
library(ggokabeito) # colour blind palette
```
...
save_if(dotPlot_original_clusters_macrophage,
...
save_if(dotPlot_original_clusters_epicardial,
...
save_if(dotPlot_epicardial_plus,
...
save_if(dotPlot_original_clusters_endocardial,
...
save_if(dotPlot_endocardial_plus,
...
save_if(dotPlot_original_clusters_myocardial,
...
save_if(dotPlot_myocardial_plus,
...
save_if(dotPlot_original_clusters_mesenchymal,
...
save_if(dotPlot_mesenchymal_plus,
...
save_if(dotPlot_original_clusters_vsmc,
...
save_if(dotPlot_original_clusters_rbc,
...
save_if(dotPlot_original_clusters_platelet,
...
save_if(dotPlot_markers,
       filename = file.path(out_dir, "05-secondary_QC", "05-dotPlot_markers.png"),
       width = 16,
       height = 8)

dotPlot_markers_ordered <- DotPlot(re_embed_no_low.qual,
                                   features = c(mesenchymal_genes,
                                                myocardial_genes,
                                                endocardial_genes,
                                                epicardial_genes,
  ...
  NoLegend() +
  scale_y_discrete(limits = c("0", "1", "3", "4", "5", "11", "12","2", "10", "14", "6", "15", "18", "19", "22", "9", "17", "7", "8", "16", "13", "20", "21", "23"))  # custom order to best align clusters of common cell types
save_if(dotPlot_markers_ordered,
       filename = file.path(out_dir, "05-secondary_QC", "05-dotPlot_markers_ordered.png"),
       width = 16,
       height = 8)
...
DimPlot(re_embed_no_low.qual,
        ...
        pt.size = 0.4) +
  NoLegend() +   # remove legend if too many clusters
  labs(title = "Sophie's manual cell type annotation",
       caption = "Filtered: <10% MT, singlets only, nFeatures >0.5%, no low quality")
