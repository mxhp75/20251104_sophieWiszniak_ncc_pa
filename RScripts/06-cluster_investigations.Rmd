...
```{r}
re_embed_interesting_cells_0.6 <- readRDS(file = file.path(out_dir, "re_embed_interesting_cells_0.6.rds"))
...
# plot the umap with new cluster numbers and colours
umap_sophieCluster <- DimPlot(re_embed_interesting_cells_0.6,
                              reduction = "umap",
...
save_if(umap_sophieCluster,
        filename = file.path(out_dir, "06-umap.re_embed_interesting_cells.sophie_anno.0.6.png"),
        width = 8,
        height = 8)
...
# Explore clusters 9 and 17

```{r}
cluster_17 <- subset(re_embed_interesting_cells_0.6@meta.data, seurat_clusters %in% 17)
...
DimPlot(re_embed_interesting_cells_0.6,
        ...
        cells.highlight = WhichCells(re_embed_interesting_cells_0.6, idents = "17"),
        cols.highlight = 'red',
        cols = "grey80",
        sizes.highlight = 1.2,
        pt.size = 0.6) +
...
# Find markers: cluster 9 vs cluster 17 only
markers_9_vs_17 <- FindMarkers(
  re_embed_interesting_cells_0.6,
  ident.1 = "9",                # positive markers = upregulated in 9
...
markers_9_vs_17$gene <- rownames(markers_9_vs_17)

# Optional: filter to significant & sort by logFC or p-val
markers_9_vs_17_sig <- markers_9_vs_17 %>%
  filter(p_val_adj < 0.05) %>%
  arrange(desc(avg_log2FC))         # highest fold-change in 9 first

# View top 10
head(markers_9_vs_17_sig, 10)

# Get top 10 genes upregulated in cluster 9
top10_9vs17 <- markers_9_vs_17 %>%
  filter(avg_log2FC > 0) %>%          # upregulated in 9
  arrange(desc(avg_log2FC)) %>%
  slice_head(n = 10) %>%
  pull(gene)

# Or top 10 upregulated in 17 vs 10 (reverse)
top10_17vs9 <- markers_9_vs_17 %>%
  filter(avg_log2FC < 0) %>%
...
DotPlot(re_embed_interesting_cells_0.6,
        features = top10_9vs17,          # or top9_17vs10
        group.by = "seurat_clusters",
        dot.scale = 8,
        cols = c("lightgrey", "red"),     # low to high expression
        col.min = -2, col.max = 2) +      # adjust scale if needed
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(title = "Top markers: Cluster 9 vs 17",
       subtitle = "Upregulated in cluster 9",
       y = "Cluster") +
  NoLegend()   # optional

top_markers_both <- c(
  markers_9_vs_17 %>% filter(avg_log2FC > 0) %>% arrange(desc(avg_log2FC)) %>% slice_head(n = 8) %>% pull(gene),
  markers_9_vs_17 %>% filter(avg_log2FC < 0) %>% arrange(desc(abs(avg_log2FC))) %>% slice_head(n = 8) %>% pull(gene)
) %>% unique()
...
DotPlot(re_embed_interesting_cells_0.6,
        ...
        dot.scale = 7) +
  RotatedAxis() +
  labs(title = "Top markers: Cluster 9 vs 17 (both directions)")
```
