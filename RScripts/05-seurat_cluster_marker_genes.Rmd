...
DotPlot(re_embed_no_low.qual,
  ...
  NoLegend() +
  # scale_y_discrete(limits = c("0", "1", "3", "4", "5", "11", "12","2", "10", "14", "6", "15", "18", "19", "22", "9", "17", "7", "8", "16", "13", "20", "21", "23")) +
  RotatedAxis()
# 
...
# Visualise DotPlot using Sophie's annotation
DotPlot(re_embed_no_low.qual,
        scale = TRUE,
        features = unique(top10_celltype_markers$gene),
        group.by = "sophie_celltype_clean",
        dot.scale = 8,
        cols = c("blue", "red"),     # low to high expression
        col.min = -2, col.max = 2) +      # adjust scale if needed
    scale_y_discrete(limits = c("Mesenchyme",
                                "Epicardium",
                                "VSMC",
                                "Myocardium",
                                "Endocardium",
  ...
  labs(title = "Top markers: sophie_celltype_clean",
       subtitle = "Upregulated in cell type") +
  NoLegend()   # optional
