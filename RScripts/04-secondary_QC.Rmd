...
cat("Cells in cluster 3:", sum(re_embed_filtered$seurat_clusters == 3), "\n")
...
cat("Cells in cluster 22:", sum(re_embed_filtered$seurat_clusters == 22), "\n")
...
cat("Total low-quality cells to remove:", sum(re_embed_filtered$seurat_clusters %in% c(3, 8, 12, 18, 21, 22, 24)), "\n\n")
...
re_embed_no_low.qual <- subset(re_embed_filtered, 
                          subset = seurat_clusters %in% c(3, 8, 12, 18, 21, 22, 24), 
                          invert = TRUE)
...
# Perform secondary QC per cluster and remove remaining low quality cells
## Set MADs
...
p <- VlnPlot(re_embed_no_low.qual,
  ...
  labs(title = "Per Cluster Lower nFeature QC Thresholds",
       ...
       caption = "Filtered: <10% MT, singlets only, no low quality (3, 8, 12, 18, 21, 21, 24) old clusters") +
...
p <- VlnPlot(re_embed_no_low.qual,
  ...
  labs(title = "Per Cluster Lower nCounts QC Thresholds",
       ...
       caption = "Filtered: <10% MT, singlets only, no low quality (3, 8, 12, 18, 21, 22, 24) old clusters") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
p + geom_segment(
  ...
  geom_label(
    ...
    aes(
      ...
      label = round(threshold_featcount_dist, 2)
    ),
    color = "black",                # text color
    fill = "white",                 # background color (white box)
    label.size = 0.35,              # thickness of the border (set to 0 for no border)
