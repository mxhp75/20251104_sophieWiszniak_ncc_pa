...
# plot the umap with old cluster numbers and colours
umap_oldCluster <- DimPlot(re_embed_e11_ko,
        reduction = "umap",
        group.by = "full_dataset_clusters",
        label = TRUE,
        label.size = 6
  ...
  labs(
    title = "E11_ko - new embedding (old clusters)",
    caption = "E11_ko"
  ) +
  NoLegend()

save_if(umap_oldCluster,
        filename = file.path(out_dir, "09-umap.re_embed_e11_ko.old_clusters.png"),
        width = 8,
        height = 8)
...
umap_newCelltype <- DimPlot(re_embed_e11_ko,
  ...
  labs(
    title = "E11_ko - new embedding (new celltypes)",
    caption = "E11_ko"
  ) +
    scale_color_manual(values = celltype_cols) +
  NoLegend()
save_if(umap_newCelltype,
           filename = file.path(out_dir, "09-umap.re_embed_e11_ko.new_celltypes.png"),
           width = 8,
           height = 8)
...
endocardial_markers <- FeaturePlot(
  ...
  plot_annotation(
    title = "E11_ko Endocardial lineage markers",
    caption = "E11_ko"
  ) &
  theme_bw(base_size = 14) &
  theme(
...
endocardial_markers
save_if(endocardial_markers, 
       filename = file.path(out_dir, "09-umap.re_embed_e11_ko.endocardial_genes.png"),
       width = 16,
       height = 8)
...
epicardial_markers <- FeaturePlot(
  ...
  plot_annotation(
    title = "E11_ko Epicardial lineage markers",
    caption = "E11_ko"
  ) &
  theme_bw(base_size = 14) &
  theme(
...
epicardial_markers
save_if(epicardial_markers, 
       filename = file.path(out_dir, "09-umap.re_embed_e11_ko.epicardial_genes.png"),
       width = 16,
       height = 8)
...
save_if(gfp_e11_ko, 
       filename = file.path(out_dir, "09-umap.re_embed_gfp_e11_ko.png"),
       width = 8,
       height = 8)

# let's save here and reload in a new session
write_rds(x = re_embed_e11_ko,
          file = file.path(out_dir, "re_embed_e11_ko.rds"))
