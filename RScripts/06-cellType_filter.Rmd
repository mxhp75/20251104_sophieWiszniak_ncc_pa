...
umap_newSophieCellType <- DimPlot(re_embed_interesting_cells_0.6,
        ...
        repel = TRUE,
        label.size = 6) + 
  scale_color_manual(values = c("Myocardium" = "#E41A1C",
                                "VSMC" = "#377EB8", 
                                "Epicardium" = "#4DAF4A",
                                "Mesenchyme" = "#984EA3", 
                                "Endocardium" = "#FF7F00",
...
# write out the .csv file for Sophie
write_csv(tibble::rownames_to_column(df, "barcode"),
          file = file.path(out_dir, "06-cluster_marker_genes/cell_types_per_cluster.csv"))

# summary
summary <- with(df, (table(cluster_celltype, orig.ident))) %>% as.data.frame()
# write out the .csv file for Sophie
write_csv(summary,
          file = file.path(out_dir, "06-cluster_marker_genes/cell_types_summary.csv"))
...
ggplot(df, aes(x = orig.ident,
  ...
  labs(
    ...
    subtitle = "Cluster resolution 0.6",
...
# Visual sanity check
umap_newSophieCellType_orig.ident <- DimPlot(re_embed_interesting_cells_0.6,
        group.by = "new_celltypes",
        split.by = "orig.ident",
        label = TRUE,
        repel = TRUE,
  ...
  scale_color_manual(values = c("Myocardium" = "#E41A1C",
                                ...
                                "Other/Unknown" = "grey70")) +
  NoLegend()
save_if(umap_newSophieCellType_orig.ident,
           filename = file.path(out_dir, "06-cluster_marker_genes", "06-umap.interesting_cells.newSophieCellType.orig.ident.png"),
           width = 16,
           height = 8)
...
barchart_cluster_newSophieCellType <- ggplot(df,
       aes(x = orig.ident,
           fill = cluster_celltype)) +
  geom_bar(position = "fill") +
  ...
  scale_y_continuous(labels = scales::percent) +
      labs(
    x = "Sample",
    y = "Proportion of cells per cluster",
    fill = "Cluster and Cell type",
    title = "Relative cell type composition per sample",
    subtitle = "Cluster resolution 0.6",
    caption = "Filtered: Interesting cells"
    ) +
  theme_bw()
save_if(barchart_cluster_newSophieCellType,
           filename = file.path(out_dir, "06-cluster_marker_genes", "06-barchart.interesting_cells.cluster_newSophieCellType.png"),
           width = 16,
           height = 8)
...
umap_cluster_newSophieCellType <- DimPlot(re_embed_interesting_cells_0.6,
        group.by = "cluster_celltype",
        label = TRUE,
        repel = TRUE,
        label.size = 6) + 
  scale_color_manual(values = cluster_celltype_cols) +
  NoLegend()
save_if(umap_cluster_newSophieCellType,
           filename = file.path(out_dir, "06-cluster_marker_genes", "06-umap.interesting_cells.cluster_newSophieCellType.png"),
           width = 16,
           height = 8)
```
