...
```{r}
re_embed_interesting_cells_0.6 <- readRDS(file = file.path(out_dir, "re_embed_interesting_0.6_rename.rds"))
...
DimPlot(re_embed_interesting_cells_0.6,
  ...
  labs(
    title = "UMAP - new embedding (new clusters)",
    caption = "Filtered: Interesting cells, cluster resolution 0.6 (sophie celltype)"
  ) +
  NoLegend()
```
...
cat("Cells in cluster 8:", sum(re_embed_interesting_cells_0.6$seurat_clusters == 10), "\n")
...
cat("Total endocardial cells to subset:", sum(re_embed_interesting_cells_0.6$seurat_clusters %in% c(6, 10, 15, 16)), "\n\n")
...
re_embed_interesting_cells_0.6_endocardial <- subset(re_embed_interesting_cells_0.6, 
                          subset = seurat_clusters %notin% c(6, 10, 15, 16), 
                          invert = TRUE)
